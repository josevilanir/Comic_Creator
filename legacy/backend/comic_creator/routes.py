import os
import shutil
from flask import Blueprint, render_template, request, redirect, url_for, flash, send_file, jsonify, current_app, g
...
def _get_base_comics():
    try:
        import main as root_main
        return getattr(root_main, 'BASE_COMICS', current_app.config.get('BASE_COMICS'))
    except Exception:
        return current_app.config.get('BASE_COMICS')
    ...
    for item in os.listdir(base_dir):
        ...
        if os.path.isdir(caminho_completo_item):
            ...
            pastas.append({
                'nome': item,
                'tem_capa': tem_capa,
                'capa_url': url_for('main.visualizar_pdf', manga=item, arquivo='capa.jpg', _external=True) if tem_capa else None
            })
...
@bp.route('/toggle_lido', methods=['POST'])
def toggle_lido():
    ...
    db = get_db()
    user_id = g.user['id'] if g.user else 1 # Fallback to ID 1 if auth not fully set
    if lido:
        db.execute('INSERT OR IGNORE INTO reads (user_id, manga, chapter) VALUES (?, ?, ?)', (user_id, manga, capitulo))
    else:
        db.execute('DELETE FROM reads WHERE user_id = ? AND manga = ? AND chapter = ?', (user_id, manga, capitulo))
    db.commit()
...
def api_library():
    """API endpoint to list all mangas in the library as JSON."""
    base_dir = _get_base_comics()
    pastas = []
...
@bp.route('/api/library/<path:nome_pasta>', methods=['GET'])
def api_listar_pasta(nome_pasta):
    """API endpoint to list chapters of a specific manga as JSON."""
    base_dir = _get_base_comics()
    pasta = os.path.join(base_dir, nome_pasta)
    if not os.path.isdir(pasta):
    ...
    lidos = set()
    rows = db.execute('SELECT chapter FROM reads WHERE user_id = ? AND manga = ?', (user_id, nome_pasta)).fetchall()
    lidos = {row['chapter'] for row in rows}

    chapters = []
    for arq in arquivos:
            chapters.append({
                'filename': arq,
                'title': arq.replace('.pdf', ''),
                'url': url_for('main.visualizar_pdf', manga=nome_pasta, arquivo=arq, _external=True),
                'thumbnail': url_for('main.visualizar_pdf', manga=nome_pasta, arquivo=arq.replace('.pdf', '.jpg'), _external=True),
                'read': arq in lidos
            })

    return jsonify({'manga': nome_pasta, 'chapters': chapters})
...
def api_download():
    ...
    capitulo_str = str(data.get('capitulo', '1')).strip()
    nome_manga_key = data.get('nome_manga', '').strip()
    
    if not (base_url and capitulo_str and capitulo_str.isdigit()):
        return jsonify({'success': False, 'message': 'Dados inválidos.'}), 400

    # Lógica de sufixo PT-BR
    url_construction_suffix = ''
    string_to_check = nome_manga_key + ' ' + base_url
    if 'manga-pt-br' in string_to_check.lower():
        url_construction_suffix = '-pt-br'

    final_url = f"{base_url}{capitulo_str}{url_construction_suffix}/"

    # Extração de nome
    url_clean = base_url
    for suffix in ['capitulo-', 'chap-']:
        if url_clean.lower().endswith(suffix):
            url_clean = url_clean[:-len(suffix)]
    
    nome_bruto = extrair_nome_manga(url_clean)
    nome_final = nome_manga_key if nome_manga_key else nome_bruto
    
    # Limpeza de nome
    if nome_final and nome_final != 'Manga Desconhecido':
        nome_limpo = limpar_sufixo_manga_pt_br(nome_final)
        if nome_limpo:
            nome_final = nome_limpo
        elif not nome_limpo and (not nome_bruto or nome_bruto == 'Manga Desconhecido'):
            nome_final = 'Manga_Sem_Nome_Definido'

    try:
        resultado = baixar_capitulo_para_pdf(final_url, nome_final, capitulo_str)
        if resultado:
            return jsonify({'success': True, 'message': f"Download de '{nome_final}' cap. {capitulo_str} concluído!", 'manga': nome_final})
        else:
            return jsonify({'success': False, 'message': 'Falha no download. Verifique a URL.'}), 500
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500


@bp.route('/api/urls', methods=['GET', 'POST', 'DELETE'])
def api_urls():
    urls = carregar_urls()
    ...
    data = request.get_json()
    if request.method == 'POST':
        urls[data['nome']] = data['url']
        salvar_urls(urls)
        return jsonify({'success': True})
    
    if request.method == 'DELETE':
        if data['nome'] in urls:
            del urls[data['nome']]
            salvar_urls(urls)
        return jsonify({'success': True})


@bp.route('/api/_debug_urls_path', methods=['GET'])
def debug_urls_path():
    """Debug endpoint: returns the path used for URLS_JSON by the app."""
    return jsonify({'URLS_JSON': current_app.config.get('URLS_JSON')})
